...
from scenes.screens import TextScene, TitleScreen, GameOverScreen, Transition
from scenes.cutscene_visuals import intro_draw, cutscene1_draw, repair_intro_draw, boss_intro_draw, win_fly_draw
...
def main():
    ...
    def go_to(s):
        nonlocal state, scene, next_state, transition
        next_state = s
        transition = Transition(fade_in=False, duration=45)
    ...
    def start_state(s):
        ...
        if s == STATE_TITLE:
            ...
            scene = TextScene(INTRO_LINES, DARK_GREY, "ERROR 1969", intro_draw)
        elif s == STATE_CUTSCENE1:
            scene = TextScene(CUTSCENE1_LINES, DARK_GREY, "CRASH LANDING", cutscene1_draw)
        elif s == STATE_LEVEL1:
            scene = Level1()
        elif s == STATE_CUTSCENE2:
            scene = TextScene(CUTSCENE2_LINES, DARK_GREY, "THE SHIP", repair_intro_draw)
        elif s == STATE_LEVEL2:
            scene = Level2()
        elif s == STATE_CUTSCENE3:
            scene = TextScene(CUTSCENE3_LINES, DARK_GREY, "THE MOON EATER", boss_intro_draw)
        elif s == STATE_LEVEL3:
            scene = Level3()
        elif s == STATE_WIN_SCENE:
            scene = TextScene(WIN_LINES, COSMIC_BLU, "VICTORY", win_fly_draw)
        elif s == STATE_WIN:
            scene = GameOverScreen(win=True)
        elif s == STATE_GAMEOVER:
            scene = GameOverScreen(win=False)

    start_state(STATE_TITLE)
    ...
    while running:
        ...
        if state == STATE_TRANSITION:
            ...
            if transition.done:
                ...
                # Fade in
                transition = Transition(fade_in=True, duration=45)
                # Brief second fade-in phase
        else:
            scene.update()
            scene.draw(screen)
